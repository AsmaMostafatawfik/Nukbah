...
type ActiveTab = 'profile' | 'courses' | 'subscription' | 'help' | 'grades';
...
export default function StudentProfilePage() {
  ...
  useEffect(() => {
    const fetchGrades = async () => {
      ...
      try {
        ...
        const user = JSON.parse(userData);
        const response = await fetch(
          `https://elearning1.runasp.net/api/Student/StudentExamGrades/${user.id}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  //           <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
  //         </svg>
  //         <h3 className="text-lg font-medium text-gray-500 mb-2">لا توجد تقييمات متاحة حالياً</h3>
  //         <p className="text-gray-400 mb-6">سيتم عرض درجات الاختبارات هنا عند توفرها</p>
  //       </div>
  //     );
  //   }

  //   // Group grades by course
  //   const gradesByCourse: Record<string, ExamGrade[]> = {};
  //   grades.forEach(grade => {
  //     if (!gradesByCourse[grade.courseTitle]) {
  //       gradesByCourse[grade.courseTitle] = [];
  //     }
  //     gradesByCourse[grade.courseTitle].push(grade);
  //   });

  //   return (
  //     <div className="space-y-6">
  //       {Object.entries(gradesByCourse).map(([courseTitle, courseGrades]) => (
  //         <div key={courseTitle} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
  //           <div className="p-4 sm:p-6 border-b border-gray-200">
  //             <h3 className="text-lg sm:text-xl font-bold text-gray-800">{courseTitle}</h3>
  //           </div>
  ...
  //                   <div className="flex-1">
  //                     <h4 className="font-medium text-gray-800">{grade.examTitle}</h4>
  //                     <p className="text-sm text-gray-500 mt-1">{grade.sessionTille}</p>
  //                   </div>
  //                   <div className="flex flex-col items-end">
  //                     <div className="flex items-center gap-2">
  //                       <span className="text-lg font-bold text-gray-800">{grade.totalScore}</span>
  //                       <span className="text-gray-500">/ {grade.maxScore}</span>
  //                     </div>
  //                     <div className={`mt-1 text-sm font-medium ${
  //                       grade.percentage >= 80 ? 'text-green-600' :
  //                       grade.percentage >= 50 ? 'text-yellow-600' :
  //                       'text-red-600'
  //                     }`}>
  //                       {grade.percentage}%
  //                     </div>
      ...
      {Object.entries(gradesByCourse).map(([courseTitle, courseGrades]) => (
        <div key={courseTitle} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <details className="group" open>
            <summary className="flex justify-between items-center p-4 sm:p-6 border-b border-gray-200 cursor-pointer list-none">
              <h3 className="text-lg sm:text-xl font-bold text-gray-800">{courseTitle}</h3>
  ...
  const renderTabContent = () => {
    switch(activeTab) {
      ...
      case 'grades':
        return (
          <div className="bg-white p-4 sm:p-6 rounded-xl shadow-sm border border-gray-200">
            <h2 className="text-xl sm:text-2xl font-bold text-gray-800 mb-4 sm:mb-6">التقييمات</h2>
            {renderGradesContent()}
          </div>
        );
      case 'help':
