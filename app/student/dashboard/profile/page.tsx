...
interface Course {
  id: number | string; // Updated to allow string IDs for fallback
  title: string;
  description: string;
  image: string | null;
  instructor?: string;
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      ...
      try {
        ...
        // Validate and transform course data with fallback IDs
        const validCourses = Array.isArray(data) 
          ? data.map((course, index) => ({
              ...course,
              id: course.id || `temp-${index}`, // Fallback ID if missing
  ...
  const renderTabContent = () => {
    switch(activeTab) {
      ...
      case 'courses':
        return (
          <div key="courses-content" className="bg-white p-4 sm:p-6 rounded-xl shadow-sm border border-gray-200">
