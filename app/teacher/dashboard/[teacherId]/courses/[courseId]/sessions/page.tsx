...
import { PlusCircle, Video, FileText, ChevronDown, ChevronUp, Plus, Trash2, ChevronLeft, EyeOff, Edit } from "lucide-react";
...
export default function CourseSessionsPage() {
  ...
  const navigateToUpdateVideo = (videoId: number, sessionId: number) => {
    router.push(`/teacher/dashboard/${teacherId}/courses/${courseId}/sessions/${sessionId}/videos/${videoId}/update-video`);
  };
  ...
  const navigateToUpdateExam = (examId: number, sessionId: number) => {
    router.push(`/teacher/dashboard/${teacherId}/courses/${courseId}/sessions/${sessionId}/exams/${examId}/update-exam`);
  };

  const navigateToUpdateSession = (sessionId: number) => {
    router.push(`/teacher/dashboard/${teacherId}/courses/${courseId}/sessions/${sessionId}/update-session`);
  };
  ...
  const navigateToSetVideoViews = (videoId: number,sessionId: number) => {
    router.push(
      `/teacher/dashboard/${teacherId}/courses/${courseId}/sessions/${sessionId}/videos/${videoId}/set-views`
    );
  };
    ...
    <div className="space-y-8 p-4 md:p-8" dir="rtl">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <button
          ...
          onClick={() =>
            router.push(
              `/teacher/dashboard/${teacherId}/courses/${courseId}/sessions/add-session`
            )
          }
        >
          <PlusCircle size={18} />
                ...
                >
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-sm">الترتيب: {session.order}</p>
                      <p className="text-sm">
                        {new Date(session.publishDate).toLocaleDateString()}
                      </p>
                    </div>
                    <button
                  ...
                  <div className="flex gap-2 mt-3">
                    ...
                    <button
                      onClick={() => navigateToUpdateSession(session.id)}
                      className="p-2 text-blue-600 hover:bg-blue-50 rounded"
                      title="تعديل الجلسة"
                    >
                      <Edit size={18} />
                  ...
                  {expandedSession === session.id && (
                    <div className="mt-4 space-y-4">
                      <div>
                        <h4 className="font-medium mb-2">فيديوهات الجلسة</h4>
                        {sessionVideos[session.id]?.length > 0 ? (
                          <div className="space-y-3">
                            {sessionVideos[session.id].map((video, idx) => (
                              <div
                                key={`mobile-video-${session.id}-${idx}`}
                                ...
                                <div className="flex justify-between items-start mb-2">
                                  <div>
                                    ...
                                    <p className="text-sm text-gray-600">
                                      النوع: {getVideoTypeName(video.type)} |
                                      المدة: {formatDuration(video.duration)}
                                    </p>
                                    {video.allowedViews !== undefined && (
                                      <p className="text-sm text-gray-600">
                                        المشاهدات المسموح بها:{" "}
                                        {video.allowedViews}
                                      </p>
                                    )}
                                  </div>
                                  <div className="flex gap-2">
                                    <button
                                      onClick={() => navigateToUpdateVideo(video.id, session.id)}
                                    ...
                                    <button
                                      onClick={() => navigateToSetVideoViews(video.id, session.id)}
                                      className="p-2 text-purple-600 hover:bg-purple-50 rounded"
                                      title="تحديد عدد المشاهدات"
                                    >
                                      <EyeOff size={18} />
                                    </button>
                                  </div>
                          ...
                          <p className="text-gray-500 text-sm">
                            لا توجد فيديوهات لهذه الجلسة
                          </p>
                        )}
                      </div>
                      <div>
                        <h4 className="font-medium mb-2">اختبارات الجلسة</h4>
                        {sessionExams[session.id]?.length > 0 ? (
                          <div className="space-y-3">
                            {sessionExams[session.id].map((exam, idx) => (
                              <div
                                key={`mobile-exam-${session.id}-${idx}`}
                                className="border p-3 rounded-lg"
                              >
                                <div className="flex justify-between items-start">
                                  <div>
                                    ...
                                    <p className="text-sm text-gray-600">
                                      النوع: {getExamTypeName(exam.type)} |
                                  ...
                                  <div className="flex gap-2">
                                    <button
                                      onClick={() => navigateToUpdateExam(exam.id, session.id)}
                          ...
                          <td colSpan={6} className="px-4 py-2">
                            <div className="bg-gray-50 p-4 rounded-lg space-y-6">
                              <div>
                                ...
                                {sessionVideos[session.id]?.length > 0 ? (
                                  <div className="space-y-3">
                                    {sessionVideos[session.id].map(
                                      (video, idx) => (
                                        <div
                                          key={`video-${session.id}-${idx}`}
                                          className="border p-3 rounded-lg"
                                        >
                                          <div className="flex justify-between items-start mb-2">
                                            <div>
                                            ...
                                            <div className="flex gap-2">
                                              <button
                                                onClick={() =>
                                                  navigateToUpdateVideo(video.id, session.id)
                                                }
                                                className="p-2 text-yellow-600 hover:bg-yellow-50 rounded"
                                                title="تعديل الفيديو"
                                              >
                              ...
                              <div>
                                ...
                                {sessionExams[session.id]?.length > 0 ? (
                                  <div className="space-y-3">
                                    {sessionExams[session.id].map(
                                      (exam, idx) => (
                                        <div
                                          key={`exam-${session.id}-${idx}`}
                                          className="border p-3 rounded-lg"
                                        >
                                          <div className="flex justify-between items-start">
                                            <div>
                                            ...
                                            <div className="flex gap-2">
                                              ...
                                              <button
                                                onClick={() =>
                                                  navigateToUpdateExam(exam.id, session.id)
                                                }
                                                className="p-1 text-yellow-600 hover:bg-yellow-50 rounded group relative"
                                                title="تعديل الاختبار"
                                              >
                                                <Edit size={16} />
