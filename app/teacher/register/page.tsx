...
//     setFormData(prev => ({
//       ...prev,
//       [name]: name === "Gender" || name === "Age" || name === "DepartmentId" ? parseInt(value) : value
//     }));
//   };
...
//         'القسم': formData.DepartmentId,
//         'المدرسة': formData.SchoolName,
//         'نبذة عنك': formData.Bio
//       };
...
//         throw new Error("رقم الهاتف يجب أن يتكون من 11 رقم");
//       }

//       // Validate age range
//       if (formData.Age < 22 || formData.Age > 100) {
//         throw new Error("العمر يجب أن يكون بين 22 و 100 سنة");
//       }
...
//         `${API_URL}/api/Account/TeacherRegister`,
//         {
//           method: 'POST',
//           body: formDataToSend,
//         }
...
//                 انضم إلى منصتنا التعليمية كمدرس
//               </h2>
...
export default function TeacherRegisterPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    SchoolName: "",
    Image: null as File | null,
    IdImage: null as File | null, // New field for ID image
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const requiredFields = {
        ...
        'المدرسة': formData.SchoolName,
        'نبذة عنك': formData.Bio,
        'صورة الهوية': formData.IdImage // ID image is now required
      };

      for (const [fieldName, value] of Object.entries(requiredFields)) {
        if (!value?.toString()?.trim() && typeof value !== 'number' && !(value instanceof File)) {
          throw new Error(`حقل ${fieldName} مطلوب`);
        }
      }
      ...
      // ID image (required)
      if (formData.IdImage) {
        formDataToSend.append('IdImage', formData.IdImage, formData.IdImage.name);
      }
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                />
                {formData.IdImage && (
                  <p className="text-sm text-green-600 mt-1">تم اختيار صورة الهوية</p>
                )}
                <p className="text-xs text-gray-500 mt-1">يجب أن تكون صورة واضحة للهوية الشخصية أو جواز السفر</p>
              </div>
            </div>
